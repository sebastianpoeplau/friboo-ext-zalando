(ns org.zalando.stups.friboo.zalando-internal.user
...
(defcommand
  ...
  (:body (http/get (utils/conpath team-service-url "/api/accounts/aws")
...
(defcommand
  ...
  (let [robot (if (and (not (clojure.string/blank? prefix))
                ...
                user-id)]
    (-> (http/get (utils/conpath kio-url "/apps/" robot)
                  {:oauth-token access-token
                   :as :json})
        (get-in [:body :team_id]))))
